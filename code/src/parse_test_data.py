...
#Date: Nov, 2013
#Filename: parse_test_data.py
#Description:
#Parses Test data into relevant fields. This chooses only data with test label to parse and writes output to a file. 
#This is very similar to extract_training_data, but has an extra field to parse (train / test type)
...
def writeSessionData(session, fid):
  ...
  for query in queries:
    #USER_ID, SESSION_ID, SERP_ID, QUERY_ID, TYPE, QUERY_TIME_PASSED, QUERY_TERMS, URLS, DOMAINS, CLICK_TIME_PASSED, DWELL_TIME
    record = "%s\t%s\t%s\t%s\t%s\t%s\t%s"%(userID, 
      query["SESSION_ID"], query["SERP_ID"], query["QUERY_ID"], query["TYPE"], query["QUERY_TIME_PASSED"], query["QUERY_TERMS"])
...
def extractTrainingData(filename, tableName):
  ...
  #Write the header
  wfid.write("USER_ID\tSESSION_ID\tSERP_ID\tQUERY_ID\tTYPE\tQUERY_TIME_PASSED\tQUERY_TERMS\tURLS\tDOMAINS\tCLICK_TIME_PASSED\tDWELL_TIME\n")
  ...
  for line in fid:
    ...
    if sessionId == currentSessionId:
      ...
      if typeOfRecord == QUERY_TYPE or typeOfRecord == TEST_TYPE:
        queryAttr = parseQueryRecord(tokens)
        queryAttr["TYPE"] = ("TRAIN" if typeOfRecord == QUERY_TYPE else "TEST")
        if lastClick:
